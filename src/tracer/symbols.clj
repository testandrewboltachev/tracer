...
and or -> assert with-out-str with-in-str with-open 
...
= not= not nil? false? true? complement identical? 
...
(def symbols
  (->>
    (clojure.string/split symbols1 #"\s+")
    (map symbol)
    set))
